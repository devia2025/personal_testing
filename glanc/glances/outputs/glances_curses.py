...
class _GlancesCurses:
    ...
    def _display_plugin_programlist(self, plugin, display_opt, max_width):
        """Display the program list plugin."""
        
        if plugin['stats'] is None:
            return []

        res = []
        stats = plugin['stats']

        # Get the sort key from the plugin
        sort_key = plugin.get('sort_key', 'cpu_percent_total')
        ...
        res.append(self.curse_add_line(header, "TITLE"))

        # Process list
        for prog in self.stats[:self.nb_lines]:

            # Get the color based on thresholds
            if prog['status'] == 'CRITICAL':
                color = 'CRITICAL'
            elif prog['status'] == 'WARNING':
                color = 'WARNING'
            else:
                color = 'OK'
            ...
            res.append(self.curse_add_line(line, color))
...
# End of glances/outputs/glances_curses.py
