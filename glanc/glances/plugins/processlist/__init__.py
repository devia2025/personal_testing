...
class Plugin(GlancesPluginModel):
    """Glances process list plugin.
    
    stats: a list of dict that contains processes information.
    """
    ...
    def __init__(self, args=None, config=None):
        ...
        # Set the default view mode - Default to program view
        self.view = True
        ...
        # Process filter key, for auto filter
    ...
    def load_config(self, config):
        """Load the configuration for the process list."""

        if config is not None and config.has_section('processlist'):

            # Default view mode
            if config.has_option('processlist', 'default_view'):
                self.view = config.get('processlist', 'default_view')
                logger.info(f"Process list default view set to: {self.view}")

            # Sort key
            if config.has_option('processlist', 'sort_key'):
                self.sort_key = config.get('processlist', 'sort_key')
                logger.info(f"Process list sort key set to: {self.sort_key}")
    ...
    def update(self):
        ...
        # Get process list from the common processes module
        if glances_processes.disable_process:
            return self.stats

        # Get the processes based on view mode
        try:
            if self.view == 'program':

                # Use programs aggregator for program view
                self.stats = glances_programs.get_programs(
                    sort_key=self.sort_key
                    .replace('cpu_percent', 'cpu_percent_total')
                    .replace('memory_percent', 'memory_percent_total'),
                    filter=self.process_filter,
                    filter_key=self.process_filter_key,
                    pinned=self.pinned_programs
                )
            else:
                # Use regular process list for thread/other views
                self.stats = glances_programs.get_processes(
                    sort_key=self.sort_key,
                    filter=self.process_filter,
                    filter_key=self.process_filter_key,
                    pinned=self.pinned_processes
                )
        except Exception as log:
            logger.error(f"Error getting process list: {log}")
            self.stats = []
    ...
    def set_view(self, view):
        """Set the view mode (process, thread, program, ...)."""

        if view in ['process', 'thread', 'program']:

            self.view = view
            logger.debug(f"Process list view set to: {view}")
            return True
    ...
    def set_sort_key(self, sort_key):
        ...
        logger.debug(f"Process list sort key set to: {sort_key}")

    def set_filter(self, filter_string):
        """Set the process filter."""

        self.process_filter = filter_string if filter_string != '' else None
        logger.debug(f"Process list filter set to: {filter_string}")

    def set_filter_key(self, filter_key):
        """Set the process filter key."""

        self.process_filter_key = filter_key
        logger.debug(f"Process list filter key set to: {filter_key}")
        
    def pin_process(self, pid):
        """Pin a process by PID."""

        if pid not in self.pinned_processes:
            self.pinned_processes.append(pid)
            logger.debug(f"Process {pid} pinned")
            return True
        return False
        
    def unpin_process(self, pid):
        """Unpin a process by PID."""

        if pid in self.pinned_processes:
            self.pinned_processes.remove(pid)
            logger.debug(f"Process {pid} unpinned")
            return True
        return False

    def pin_program(self, program_name):
        """Pin an entire program by name."""
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        # Sould be implemented in the TUI part
        return []

# End of glances/plugins/processlist/__init__.py
